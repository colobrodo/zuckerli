...
ABSL_FLAG(bool, tsv, false, "Porcellain-like option: If true the output is provided as tsv, with three field:"
  " the first for the iteration number, followed by the total time of this run and lastly the run's average time (in ns/arc)");

void TimedRandomAccess(zuckerli::CompressedGraph graph, uint32_t random, uint32_t repeats, uint32_t seed, bool print_as_tsv) {
  std::mt19937 mt(seed);
  ...
  if (!print_as_tsv) std::cout << "Random access to adjacency lists..." << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  TimedRandomAccess(graph, absl::GetFlag(FLAGS_random), absl::GetFlag(FLAGS_repeats), absl::GetFlag(FLAGS_seed), print_as_tsv);
  return 0;
}